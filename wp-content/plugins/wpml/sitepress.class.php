...
class SitePress
{
	...
	function language_filter()
	{
		...
		if ( empty( $this->settings[ 'hide_how_to_translate' ] ) && $type == 'page' && !$this->get_icl_translation_enabled() ) {
			$prot_link = '<span id="icl_how_to_translate_link" class="button" style="padding-right:3px;" ><img align="baseline" src="' . ICL_PLUGIN_URL . '/res/img/icon16.png" width="16" height="16" style="margin-bottom:-4px" /> <a href="https://wpml.com/?page_id=3416">' . __( 'How to translate', 'sitepress' ) . '</a><a href="#" title="' . esc_attr__( 'hide this', 'sitepress' ) . '" onclick=" if(confirm(\\\'' . __( 'Are you sure you want to remove this button?', 'sitepress' ) . '\\\')) jQuery.ajax({url:icl_ajx_url,type:\\\'POST\\\',data:{icl_ajx_action:\\\'update_option\\\', option:\\\'hide_how_to_translate\\\',value:1,_icl_nonce:\\\'' . wp_create_nonce( 'update_option_nonce' ) . '\\\'},success:function(){jQuery(\\\'#icl_how_to_translate_link\\\').fadeOut()}});return false;" style="outline:none;"><img src="' . ICL_PLUGIN_URL . '/res/img/close2.png" width="10" height="10" style="border:none" alt="' . esc_attr__( 'hide', 'sitepress' ) . '" /><\/a>' . '<\/span>';
		} else {
			$prot_link = '';
		}
		?>
				...
				switch ( $iclTranslationManagement->settings[ 'doc_translation_method' ] ) {
					case ICL_TM_TMETHOD_EDITOR:
						...
						if ( !$current_user_is_translator ) {
							...
							$current_user_is_translator = $iclTranslationManagement->is_translator( $this->get_current_user()->ID, $args );
							if ( $current_user_is_translator ) {
								$alt = sprintf( __( "You can't edit this translation because you're not the translator. <a%s>Learn more.</a>", 'sitepress' ), ' href="https://wpml.com/?page_id=52218"' );
							} else {
								$alt = sprintf( __( "You can't edit this translation because you're not a %s translator. <a%s>Learn more.</a>", 'sitepress' ), $v[ 'display_name' ], ' href="https://wpml.com/?page_id=52218"' );
							}
					...
					case ICL_TM_TMETHOD_EDITOR:
						...
						if ( $job_id ) {
							...
							if ( $current_user_is_translator ) {
								...
								$link = '#';
								$img  = 'add_translation_disabled.png';
								$alt  = sprintf( __( "You can't add this translation because you're not a %s translator. <a%s>Learn more.</a>", 'sitepress' ), $v[ 'display_name' ], ' href="https://wpml.com/?page_id=52218"' );
							}
						}
	...
	function display_wpml_footer()
	{
		if ( $this->settings[ 'promote_wpml' ] ) {
			...
			if ( in_array( ICL_LANGUAGE_CODE, array( 'ja', 'zh-hans', 'zh-hant', 'ko' ) ) ) {
				...
				echo '<p id="wpml_credit_footer">' . sprintf( __( '<a href="%s"%s>Multilingual WordPress</a> by <a href="%s" rel="nofollow">ICanLocalize</a>', 'sitepress' ), 'http://www.icanlocalize.com/site/' . $cl_icl, 'https://wpml.com/' . $cl, $nofollow_wpml ) . '</p>';
			} else {
				echo '<p id="wpml_credit_footer">' . sprintf( __( '<a href="%s"%s>Multilingual WordPress</a> by <a href="%s" rel="nofollow">ICanLocalize</a>', 'sitepress' ), 'https://wpml.com/' . $cl, $nofollow_wpml, 'http://www.icanlocalize.com/site/' . $cl_icl ) . '</p>';
			}
		}
	}
	...
	public static function check_settings_integrity() {
		...
		if ( (!$setup_complete || !$default_language) && $existing_translations ) {
			...
			$message .= '</p>';
			$message .= '<p>';
			$message .= sprintf( __( 'If you have just upgraded WPML or after starting over you keep getting this message, please contact the <a href="%s">support forum</a> as soon as possible, in order to provide you with a fix to this issue.', 'sitepress' ), 'https://wpml.com/forums/' );
			$message .= '</p>';
			$message .= '<p>';
			...
			$message .= '&nbsp;';
			$message .= '&nbsp;';
			$message .= '<a href="https://wpml.com/forums/" class="button">' . __('Contact Support','sitepress') . '</a>';
			$message .= '</p>';
			$message .= '<p>';
